/******************************************************************************
 * File: english_corner.pov
 * Description:
 *      Example of the Brickwall system using a stretcher layout
 ******************************************************************************/
...
//-----------------------------------------------------------------------------
// Scene_wall_object(Layout)
//
#macro Scene_wall_object(Layout)
    #local _bricks_shape_fn = Brickwall_english_bricks_shape_fn(Layout,Scene_brick_shape)
    #local _mortar_block    = Brickwall_english_mortar_block_create(Layout,Scene_mortar_inset)
    #local _mortar_shape_fn = Brickwall_english_mortar_shape_fn(Layout,_mortar_block,Common_mortar_joint_spec,_bricks_shape_fn)
    
    #local _lbounds         = <0,0,0>;
    #local _ubounds         = Layout.wall_sz;
    ...
    #local _bricks  = isosurface {
        ...
        max_gradient 10
        material {
            texture { Common_bricks_texture(Layout) }
            interior { ior 1.5 }
        }        
    }
...
#macro Scene_default_object()
    ...
    #local _wall_spec.left_end_type     = Brickwall_end_type_corner;
    #local _front_layout                = Brickwall_english_layout_create(_wall_spec,Common_brick_spec,0);
    
    #local _front_wall  = Scene_wall_object(_front_layout)
    
    #local _wall_spec                   = Brickwall_wall_spec_create(Scene_wall_length,Scene_wall_courses,Scene_wall_halfbricks);
    #local _wall_spec.right_end_type    = Brickwall_end_type_corner;
    #local _side_layout                 = Brickwall_english_layout_create(_wall_spec,Common_brick_spec,1);
    
    #local _side_wall   = Scene_wall_object(_side_layout)
    ...
    #local _result  = union {
        ...
        object {
            _side_wall
            translate <-_side_layout.wall_sz.x, 0, 0>
            rotate <0, 90, 0>
        }    
        translate <-_front_layout.wall_sz.x/2, 0, 0>
    }
...
#switch(val(str(Scene_View,0,2)))
    #case(1.1)
        ...
        #declare camera_location    = <0, 2*Scene_wall_courses*Common_brick_spec.unit_sz.y, -0.8*Scene_wall_length>;
        #declare camera_lookat      = <0, Scene_wall_courses*Common_brick_spec.unit_sz.y, 0>;
    #break
